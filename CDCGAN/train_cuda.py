...
def parse_arguments():
    ...
    parser.add_argument(
        "--niter", type=int, default=10, help="number of epochs to train for" # todo 5000
    )
    parser.add_argument(
        "--lrD",
        type=float,
...
def train(
    ...
    if opt.cuda:
        ...
        conditional_channels = torch.tensor(conditional_channels)
        conditional_channels.to(device)
        org_data.cuda() # todo fix because org_data is too large
    ...
    for epoch in range(opt.niter):
        ...
        while i < num_batches:  # len(dataloader):
            ...
            while j < Diters and i < num_batches:  # len(dataloader):
                ...
                if opt.cuda:
                    context_frame.to(device), out_frame.to(device)
                joined_frame = torch.cat((context_frame, out_frame), dim=3)
                assert joined_frame[0].shape == (z_dims, 32, 32)
                input.resize_as_(joined_frame).copy_(joined_frame)
                inputv = Variable(input)


                errD_real = netD(inputv.to(device)).mean(0).view(1)
                errD_real.backward(one)

                # train with fake
                noise.resize_(opt.batchSize, 1, 16, 16).normal_(0, 1)

                ref_idx = torch.randperm(len(org_data))[: opt.batchSize]
                ref_frames = org_data[ref_idx].prev_frame

                ref_frames.to(device)

                gen_input = torch.cat(
                    (noise.to(device), ref_frames[:, conditional_channels, 8:-8, :]), dim=1
                )

                gen_input.to(device) # todo
                # gen_input = ref_frames[:, :, 8:-8, :]

                noisev = Variable(gen_input, volatile=True)  # totally freeze netG
                fake = Variable(netG(noisev.to(device)).data) # todo
                # fake = Variable(netG(noisev).data)
                stitched = torch.cat(
                    (ref_frames.to(device), fake[:, :, :, 16:]), dim=3 # todo
                    # (ref_frames, fake[:, :, :, 16:]), dim=3
                )  # stitch the context frame
                assert stitched[0].shape == (z_dims, 32, 32)

                inputv = stitched
                errD_fake = netD(inputv.to(device)).mean(0).view(1)
            ...
            # fake = netG(g_input.to(device)) #todo
            fake = netG(g_input)
            # fake[:, :, :, :16] = ref_frames
            # stitched = torch.cat((ref_frames.to(device), fake[:, :, :, 16:]), dim=3) #todo
            stitched = torch.cat((ref_frames, fake[:, :, :, 16:]), dim=3)
            # if opt.cuda:
            #     stitched.to(device)
            # errG = netD(stitched.to(device)).mean(0).view(1) #todo
            ...
            if gen_iterations % 5000 == 0:  # was 500
                with torch.no_grad():
                    fixed_noise.resize_(opt.batchSize, 1, 16, 16)
                    ref_idx = torch.randperm(len(org_data))[: opt.batchSize]
                    ref_frames = org_data[ref_idx].prev_frame
                    ref_frames.to(device) # todo
                    gen_input = torch.cat(
                        (fixed_noise.to(device), ref_frames[:, conditional_channels, 8:-8, :].to(device)), #todo
                        # (fixed_noise, ref_frames[:, conditional_channels, 8:-8, :]),
                        dim=1,
                    )
                    # gen_input = ref_frames[:, :, 8:-8, :]
                    fake = netG(Variable(gen_input.to(device), volatile=True)) # todo
                    # fake = netG(Variable(gen_input, volatile=True))
                stitched = torch.cat((ref_frames.to(device), fake[:, :, :, 16:].to(device)), dim=3) # todo
                # stitched = torch.cat((ref_frames, fake[:, :, :, 16:]), dim=3)
                ...
                torch.save(
                    netD.state_dict(),
                    "{0}/netD_epoch_{1}_{2}_{3}.pth".format(
                        opt.experiment, gen_iterations, opt.problem, opt.nz
                    ),
                )
...
def main():
    ...
    netG = Generator(
        latent_size=(len(conditional_channels) + 1, 16, 16), out_size=(CHANNEL_SIZE, 32, 32)
    )
    print(netG)
    ...
    netD = DCGANDiscriminator(input_size=(CHANNEL_SIZE, 32, 32), apply_sigmoid=False)
    print(netD)
