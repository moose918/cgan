...
class MEOptimizer():
    ...
    def tell(self, objective_values, behavior_values):
        ...
        self.logbook.record(iteration=self.i, containerSize=self.grid.size_str(), evals=len(self.inds),
                            nbUpdated=nb_updated, elapsed=timer() - self.start_time, meanFitness=meanFitness,
...
def simulate(
    ...
    if not ENV3D:
        final_levels = np.empty(shape=(init_states.shape[0], env.unwrapped._prob._height, env.unwrapped._prob._width),
                                dtype=np.uint8)
    else:
        final_levels = np.empty(shape=(
        init_states.shape[0], env.unwrapped._prob._height, env.unwrapped._prob._width, env.unwrapped._prob._length),
                                dtype=np.uint8)
    batch_reward = 0
...
class EvoPCGRL:
    ...
    def infer(self, concat_gifs=True):
        assert INFER
        self.door_coords = None if not hasattr(self,
                                               'door_coords') else self.door_coords  # HACK for backward compatibility
        args = self.args
        self.init_env()
        archive = self.gen_archive
        if args.algo == "ME":
        ...
        if EVALUATE:
            ...
            if not CMAES:
                ...
                plot_score_heatmap(diversity_scores / 10, "diversity", self.bc_names, **plot_args,
                                   bcs_in_filename=False)
    ...
    if n_init_states == 0:
        ...
        if not ENV3D:
            if CONTINUOUS:
                ...
                init_states[0, :, height // 2 - sh // 2:height // 2 + sh // 2,
                width // 2 - sw // 2: width // 2 + sw // 2] = 1
            else:
